...
class ScoreMaker(nn.Module):
    ...
    def forward(self, x):
        """
        x: [num_layer, bz, seq_len, dim]
        """
        x = x.permute(1,2,0,3) # [bz, seq_len, num_layer, dim]
        scores = (self.Q(x[:,:,-1,:]).unsqueeze(2) @ self.K(x).permute(0,1,3,2))/torch.sqrt(torch.tensor(x.shape[-1], dtype=torch.float)) #[bz, seq_len, 1, num_layer]
        scores = nn.functional.softmax(scores.squeeze(2), dim=-1).unsqueeze(-1) #[bz, seq_len, num_layer, 1]
        x = torch.sum(self.V(x) * scores, dim=2) # [batch, seq_len, dim]
...
class RobertaForQuestionAnswering(BertPreTrainedModel):
  ...
  def __init__(self, config):
    ...
    self.roberta = RobertaModel(config)
    self.score_maker = ScoreMaker(config.hidden_size)
    self.output_hidden_states = config.output_hidden_states
